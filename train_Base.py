...
import numpy as np
from src import MIR1K, cycle, summary, JM_Base, FL, mae
...
def train(weight_svs):
    ...
    gamma = 0
    # weight_svs = 10
    weight_pe = round(2 - weight_svs, 2)
    in_channels = 1
    n_blocks = 1
    latent_layers = 1
    seq_l = 2.56
    ...
    logdir = 'runs/MIR1K_Base/' + 'nblocks' + str(n_blocks) + '_latent' + str(latent_layers) + '_frames' + str(seq_frames) \
    ...
    if resume_iteration is None:
        ...
        model = JM_Base(in_channels, n_blocks, hop_length, latent_layers, seq_frames)
...
for weight_svs in [1.2, 0.8, 1.4, 0.6, 1.6, 0.4, 1.8, 0.2, 1.0]:
    train(weight_svs)
