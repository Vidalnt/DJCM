...
for _, row in tqdm(df_info.iterrows()):
    ...
    audio_m, sr = librosa.load(os.path.join(path_in_root, filename), sr=16000, mono=True)
    
    out_filename = filename.replace('/', '_').replace('\\', '_').replace('.wav', '')
    ...
    np.savetxt(pv_out, f0_hz, fmt="%.6f")